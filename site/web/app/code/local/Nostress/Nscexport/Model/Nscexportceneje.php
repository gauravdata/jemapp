 ...
 *
 * @category   Nostress
 * @package    Nostress_Nscexportceneje
 */
define("IS", '","'); //item separator

class Nostress_Nscexport_Model_Nscexportceneje extends Mage_Core_Model_Abstract
{	
    ...
    {
        parent::_construct();
        $this->_init('nscexport/nscexportceneje');
    }

	public function generateXml($nscexportId)
	{
	  	$encoding = "UTF-8";//encoding of xml file	
	  	$xmlHead = array('id','title','price','currency','brand', 'sku', 'url','stock','qty','description','ean','shipping','image','category', 'mpn'); 
    ...
    {       
        $result ='"';        
        $result .= Nostress_Nscexport_Helper_Data::formatProductAttributePipe(true,$this->model->getProductCurrentId(),IS);
        ...
        $result .= Nostress_Nscexport_Helper_Data::formatProductAttributePipe(true,$this->model->getProductCurrency(),IS);        //final price 
        $result .= Nostress_Nscexport_Helper_Data::formatProductAttributePipe(true,$this->model->getProductManufacturer(),IS);
        ...
        $result .= Nostress_Nscexport_Helper_Data::formatProductAttributePipe(true,$this->model->getProductQuantity(),IS);        //qty
        ...
        $result .= Nostress_Nscexport_Helper_Data::formatProductAttributePipe(true,$this->model->getProductCategoryName(),IS);        //category
        $result .= Nostress_Nscexport_Helper_Data::formatProductAttributePipe(true,$this->model->getProductTextAtribute('mpn'),IS); //mpn                                                      
        $result .="\"\r\n";
