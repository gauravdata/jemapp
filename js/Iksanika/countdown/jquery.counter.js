...
!(function (context, definition) {
  if (typeof define == 'function' && typeof define.amd  == 'object') define(['jquery'], definition);
    ...
    var checkStop = function(data) {
        var stop = 0;
        var current = 0;
        $j.each(data.parts, function(i, part) {
    ...
    var refresh = function(e, to) {
        ...
        while (i >= to) {
            ...
            $j.each(split(digits, ''), animateIJ);
            i--;
        }
    };

    var animate = function(e, ipart, idigit, digit) {
        var edigit = $j($j(e.children('span.part').get(ipart)).find('span.digit').get(idigit));
    ...
    var split = function(undef) {
        ...
        self = function (str, separator, limit) {
            ...
            if (!compliantExecNpcg) {
                ...
                separator2 = new RegExp("^" + separator.source + "$j(?!\\s)", flags);
            }
    ...
    var methods = {
        init: function(options) {
            ...
            return this.each(function() {
                ...
                e.html('');
                $j.each(format, function(index, value) {
                    if (/^\d+$/.test(value)) {
                        var part = {};
                        part.index = index;
                        ...
                        var epart = $j('<span>').addClass('part').addClass('part' + index);
                        var digits = part.value + '';
                        while (digits.length < part.padding) {
                            digits = '0' + digits;
                        }
                        $j.each(split(digits, ''), function(dindex, dvalue){
                            epart.append($j('<span>').addClass('digit digit' + dvalue).text(dvalue));
                        });
                        e.append(epart);
                        data.parts.push(part);
                    } else {
                        e.append($j('<span>').addClass('separator').addClass('separator' + index).text(value));
                    }
                });
                if (!checkStop(data)) {
                    data.intervalId = setInterval($j.proxy(tick, this), data.interval);
                } else {
                    e.trigger("counterStop");
                }
                e.data('counter', data);
        ...
        reset: function() {
            return this.each(function() {
                var e = $j(this);
                var data = e.data('counter');
                $j.each(data.parts, function(pindex, pvalue){
...
//    $j.fn.counter = function(method) {
    $j.fn.counter = function(method) {
        ...
        if (methods[method]) {
            ...
            $j.error('Method ' +  method + ' does not exist on jQuery.counter');
        }
    };
