<div id="category" class="content">
	<p class="chemin">
		<a href="./">Home</a> >> <span class="current_page">New products</span>
	</p>
	{if !isset($category)}
		<p class="noProd">This category cannot be loaded.</p>
	{else}
		<h1>New products<span> {$category.productNumber} products<span></h1>
		
		<ul id="subCategories">
			{foreach from=$categoriesSub item=subchildren}
				<li>
					<a href="./category.php?{$subchildren.lien}" title="{$subchildren.nom}"><img src="{$shop_url}img/c/{$subchildren.id}-medium.jpg" alt="{$subchildren.nom}"/></a>
					<h3><a href="./category.php?{$subchildren.lien}" title="{$subchildren.nom}">{$subchildren.nom|truncate:30}</a></h3>
				</li>
			{/foreach}
		</ul>
	...
	<p class="pagination">
		{if $category.productNumber>$config.number_product}
			<span>
						{if $page>1}
							<a href="new-products.php?page={$page-1}"><< Previous</a>
						{/if}
						|
						{section name=pagination start=1 loop=$category.productNumber/$config.number_product+2 step=1}
							...
							{else}
								<span class="page"><a href="new-products.php?page={$smarty.section.pagination.index}">{$smarty.section.pagination.index}</a></span> |
							{/if}
						{/section}
						{if ($page*$config.number_product)<$category.productNumber}
							<a href="new-products.php?page={$page+1}">Next >></a>
						{/if}
			</span>
		{/if}
	</p>
