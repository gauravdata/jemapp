<div id="category" class="content">
	<p class="chemin">
		<a href="./">Home</a> >> <span class="current_page">Promotions / soldes</span>
	</p>
	{if !isset($category)}
		<p class="noProd">This category cannot be loaded.</p>
	{else}
		<h1>Sales<span> {$category.productNumber} products<span></h1>
	...
	<p class="pagination">
		{if $category.productNumber>$config.number_product}
			<span> 
						{if $page>1}
							<a href="prices-drop.php?page={$page-1}"><< Previous</a>
						{/if}
						|
						{section name=pagination start=1 loop=$category.productNumber/$config.number_product+2 step=1}
							...
							{else}
								<span class="page"><a href="prices-drop.php?page={$smarty.section.pagination.index}">{$smarty.section.pagination.index}</a></span> |
							{/if}
						{/section}
						{if ($page*$config.number_product)<$category.productNumber}
							<a href="prices-drop.php?page={$page+1}">Next >></a>
						{/if}
			</span>
		{/if}
	</p>
