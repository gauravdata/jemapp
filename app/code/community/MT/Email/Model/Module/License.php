...
class MT_Email_Model_Module_License
{
    ...
    public function check()
    {
        ...
        $response = $this->request(array(
            Mage::app()->getRequest()->getParam('section') => base64_encode($key)
        ));
        $requestStatus = $response->getStatus();
        $responseBody = json_decode($response->getRawBody());
        ...
        if ($requestStatus == 200) {
            if ($responseBody[0]->status != 1) {
                Mage::getSingleton('adminhtml/session')->addError('Error: '.$responseBody[0]->error.' Please go to "System >> Configuration >> MageTrend Extensions >> Licenses" and add the license key.');
    ...
    {
        return Mage::getStoreConfig('mtlicense/'.Mage::app()->getRequest()->getParam('section').'/key');
    }
