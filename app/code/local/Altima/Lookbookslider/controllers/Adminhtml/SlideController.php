...
class Altima_Lookbookslider_Adminhtml_SlideController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('cms/lookbookslider');
    }
    ...
    public function uploadAction() {
        ...
        if ($slider_id) {
            $slider_dimensions = Mage::getModel('lookbookslider/lookbookslider')->load($slider_id)->toArray(array('width', 'height', 'no_resample'));

            $upload_dir = Mage::getBaseDir('media') . DS . 'lookbookslider' . DS;

            $io->checkAndCreateFolder($upload_dir, 0755);
        ...
        if (!is_array($slideIds)) {
            ...
            try {
                $slides = Mage::getModel('lookbookslider/slide')->getCollection()
                        ->addFieldToFilter('slide_id' , array('in' => $slideIds));
                foreach ($slides as $slide){
                    $slide->setStatus($this->getRequest()->getParam('status'))
