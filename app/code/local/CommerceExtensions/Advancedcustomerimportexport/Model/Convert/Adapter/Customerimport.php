...
//                        }
    ...
    public function saveRow($importData)
    {
		...
		$skipCustomerRecord = false;
		
		if($this->getBatchParams('do_not_update_customer') == "true") { 
			if ($customer->getId()) {
        ...
        if (!$customer->getId()) {
            ...
            } else if (!isset($importData['password_hash'])) {
		...
		if($this->getBatchParams('delete_all_customer_address_first') == "true") { 
			if ($customer->getId()) {
		...
		} // END CHECK OF IF CAN IMPORT ADDRESSES TO EXISTING CUSTOMERS
        ...
        $customer->save();
		//if you want to force all users to be confirmed (assuming your using this option)
        ...
        if ($importBillingAddress && !$savedBillingAddress) {
			...
			if(isset($importData['is_default_billing'])) {
                if($importData['is_default_billing']==1) { $customer->setDefaultBilling($billingAddress->getId());} 
			}
            ...
            if ($onlyAddress) {
				...
				if(isset($importData['is_default_shipping'])) {
					if($importData['is_default_shipping']==1) { $customer->setDefaultShipping($billingAddress->getId());} 
				} else {
               	 $customer->setDefaultShipping($billingAddress->getId());
				}
                #$customer->setDefaultShipping($billingAddress->getId());
        ...
        if ($importShippingAddress && !$savedShippingAddress) {
			...
			if(isset($importData['is_default_shipping'])) {
                if($importData['is_default_shipping']==1) { $customer->setDefaultShipping($shippingAddress->getId());} 
			}
		...
		} //check on if customer is update or not
        return $this;
    }
