<?php
class Biztech_Translator_Block_Adminhtml_Config_Form_Renderer_Website extends Mage_Adminhtml_Block_System_Config_Form_Field{
        protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
        {
            ...
            $data_info = Mage::helper('translator')->getDataInfo();
           

            if(isset($data_info->dom) &&  intval($data_info->c) > 0  &&  intval($data_info->suc) == 1){
                foreach (Mage::app()->getWebsites() as $website) {                   
                    $url = $website->getConfig('web/unsecure/base_url');
                    $url = Mage::helper('translator')->getFormatUrl(trim(preg_replace('/^.*?\/\/(.*)?\//', '$1', $url)));
