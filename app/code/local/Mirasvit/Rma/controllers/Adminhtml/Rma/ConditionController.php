...
class Mirasvit_Rma_Adminhtml_Rma_ConditionController extends Mage_Adminhtml_Controller_Action
{
    /**
     * @return Mirasvit_Rma_Adminhtml_Rma_ConditionController
     */
    ...
    public function addAction()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('adminhtml')->__('Add Condition '),
            Mage::helper('adminhtml')->__('Add Condition'));
    ...
    public function editAction()
    {
        ...
        if ($condition->getId()) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('adminhtml')->__('The Condition does not exist.'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__('Condition was successfully saved'));
                Mage::getSingleton('adminhtml/session')->setFormData(false);

                if ($this->getRequest()->getParam('back')) {
                    $this->_redirect('*/*/edit', array('id' => $condition->getId(),
                        'store' => $condition->getStoreId()));
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('adminhtml')->__('Unable to find Condition to save'));
        $this->_redirect('*/*/');
    }
    ...
    public function massChangeAction()
    {
        ...
        if (!is_array($ids)) {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('adminhtml')->__('Please select Condition(s)'));
        } else {
            try {
                $isActive = $this->getRequest()->getParam('is_active');
                foreach ($ids as $id) {
                    /** @var Mirasvit_Rma_Model_Condition $condition */
    ...
    /**
     * @return Mirasvit_Rma_Model_Condition
     */
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('rma/dictionary/condition');
    }
