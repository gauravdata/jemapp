...
class AW_Points_Helper_Order extends Mage_Core_Helper_Abstract
{
    ...
    /**
     * Returns quantity of all visible credit memo items
     *
     * @param Mage_Sales_Model_Order_Creditmemo $memo
     * @return int
     */
    public function getCreditMemoVisibleItemsQty($memo)
    {
        $totalQty = 0;
        foreach($memo->getAllItems() as $item) {
            $orderItem = $item->getOrderItem();
            if ($orderItem && !$orderItem->isDeleted() && !$orderItem->getParentItemId()) {
                $totalQty += $orderItem->getQtyRefunded();
            }
        }
        return $totalQty;
    }
