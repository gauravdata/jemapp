...
class AW_Points_Model_Actions_OrderRefunded extends AW_Points_Model_Actions_OrderInvoiced
{
    ...
    public function addTransaction($additionalData = array())
    {
        ...
        // Spend refunded transaction
        $orderTransaction = $this->_transaction
                            ->getCollection()
                            ->addBalanceActiveFilter()
                            ->addOrderFilter($this->getObjectForAction()->getId())
                            ->addSummaryFilter($this->_summary->getId())
                            ->addBalanceChangeTypeFilter($this->_transaction->getBalanceChangeType())
                            ->getFirstItem()
        ;
        if ($orderTransaction->getId()) {
            $amountToReduce = -$this->_transaction->getBalanceChange();
            $itemBalance = $orderTransaction->getBalanceChange();
            $itemBalanceSpent = $orderTransaction->getBalanceChangeSpent();
            $itemBalanceDelta = $itemBalance - $itemBalanceSpent;

            if ($amountToReduce >= $itemBalanceDelta) {
                $amountToReduce -= $itemBalanceDelta;
                $orderTransaction
                    ->setBalanceChangeSpent($itemBalance)
                    ->save()
                ;
                ...
                if ($amountToReduce > 0) {
                    $this
                        ->_transaction
                        ->getCollection()
                        ->updateBalanceChangeSpent($this->_summary->getId(), -$amountToReduce);
                }
            } else {
                if ($amountToReduce > 0) {
                    $orderTransaction->setBalanceChangeSpent($itemBalanceSpent + $amountToReduce)->save();
                }
            }
        } else {
