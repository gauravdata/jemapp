    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        if ($session->getData('use_points') && $address->getBaseGrandTotal() && $isCustomerLoggedIn) {
            ...
            foreach($quote->getAllItems() as $item) {
                ...
                $baseDiscountAmount += $item->getBaseDiscountAmount() - $item->getBaseHiddenTaxAmount();
            }
            $subtotalWithDiscount = $subtotal - $discountAmount;
            $baseSubtotalWithDiscount = $baseSubtotal - $baseDiscountAmount;
            ...
            if ($moneyBaseCurrencyForPoints > $baseSubtotalWithDiscount) {
                ...
                if ($adjustTaxValues) {
                    ...
                    $origTaxAmount = $address->getData('tax_amount') - $address->getData('shipping_tax_amount');
                    $origBaseTaxAmount = $address->getData('base_tax_amount') - $address->getData('base_shipping_tax_amount');
                    ...
                    $address->setTaxAmount($newTaxAmount + $address->getData('shipping_tax_amount'));
                    $address->setBaseTaxAmount($newBaseTaxAmount + $address->getData('base_shipping_tax_amount'));
                }
