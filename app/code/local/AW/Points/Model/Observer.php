...
class AW_Points_Model_Observer extends Mage_Core_Block_Abstract
{
        ...
        if ($pointsToReturn) {
            ...
            if ($infoObject instanceof Varien_Object) {
                ...
                $comment = sprintf(
                    ...
                    Mage::helper('points/order')->getCreditMemoVisibleItemsQty($memo),
    ...
    public function orderPlaceBefore($observer)
    {
        ...
        if ($order->getCustomerId()) {
            ...
            $sum = Mage::helper('points')->getBaseSubtotalWithDiscountApplied($quote);
    ...
    public function orderLoadAfter($observer)
    {
        ...
        if ($order->getBaseGrandTotal() == 0) {
            $_forcedCanCreditmemo = false;
            foreach ($order->getAllItems() as $item) {
                if ($item->canRefund()) {
                    $_forcedCanCreditmemo = true;
                }
            }
            $order->setForcedCanCreditmemo($_forcedCanCreditmemo);
            $order->setAWPointsForcedCanCreditmemo(true);
        }
    }
            ...
            foreach ($invoice->getAllItems() as $item) {
                ...
                if ($orderItemQty) {
                    $itemToSubtotalMultiplier
                        = $item->getData('base_row_total_incl_tax') / $invoice->getOrder()->getBaseSubtotalInclTax();
                    $moneyBaseToReduceItem = $moneyBaseToReduce * $itemToSubtotalMultiplier;
                    $moneyToReduceItem = $moneyToReduce * $itemToSubtotalMultiplier;

                    if ($item->getData('base_row_total_incl_tax') + $moneyBaseToReduceItem < 0) {
                        $invoice->setMoneyForPoints(
                            $invoice->getMoneyForPoints() + $item->getData('row_total_incl_tax')
                        );
                        $invoice->setBaseMoneyForPoints(
                            $invoice->getBaseMoneyForPoints() + $item->getData('base_row_total_incl_tax')
                        );
                    } else {
    ...
    {
        $result = false;
        $collection = $this->getCollectionOfFullyPaidOrdersForCustomer($referralId);
    ...
    private function getCollectionOfFullyPaidOrdersForCustomer($customerId)
    {
        $collection = Mage::getModel('sales/order')->getCollection()->addAttributeToFilter('customer_id', $customerId);
        $this->addFullyPaidOrdersFilter($collection);
        return $collection;
    }

    private function addFullyPaidOrdersFilter($collection)
    {
        return $collection->getSelect()->where('base_total_paid = base_grand_total');
    }
    ...
    private function _getPointsForOrder($invoice)
    {
        ...
        // add  fixed count of points only for fully paid order
    ...
    private function isOrderFullyPaid($order)
    {
    ...
    public function checkPointsApplied($observer)
    {
        // AW Storecredit compatibility
        if (Mage::helper('core')->isModuleOutputEnabled('AW_Storecredit') && Mage::helper('aw_storecredit/config')->isModuleEnabled()) {
            $session = Mage::getSingleton('checkout/session');
