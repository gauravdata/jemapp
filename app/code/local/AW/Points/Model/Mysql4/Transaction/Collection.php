...
class AW_Points_Model_Mysql4_Transaction_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function addSummaryFilter($summaryId)
    {
        $this->getSelect()->where('summary_id = ?', $summaryId);
        return $this;
    }

    public function addBalanceChangeTypeFilter($balanceChangeType)
    {
        $this->getSelect()->where('balance_change_type = ?', $balanceChangeType);
        return $this;
    }
}
