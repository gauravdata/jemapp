...
class AW_Followupemail_Model_Events
{
    ...
    */
    ...
    public function newsletterSubscribe($event)
    {
        ...
        $statusSubscribed = Mage_Newsletter_Model_Subscriber::STATUS_SUBSCRIBED;
        ...
        if (Mage::helper('followupemail')->canUseAN()) {
            if (class_exists('AW_Advancednewsletter_Model_Subscriber')) {
                if ($subscriber instanceof AW_Advancednewsletter_Model_Subscriber) {
                    $statusKey = 'status';
                    $emailKey = 'email';
                    $statusSubscribed = AW_Advancednewsletter_Model_Subscriber::STATUS_SUBSCRIBED;
                    $statusUnsubscribed = AW_Advancednewsletter_Model_Subscriber::STATUS_UNSUBSCRIBED;
                }
            }
        }

        if ($subscriber->getData($statusKey) === $statusSubscribed) {
            $queue->cancelByEvent(
                $subscriber->getData($emailKey),
                AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_NEW_SUBSCRIPTION
            );
        } elseif ($subscriber->getData($statusKey) === $statusUnsubscribed) {
            $queue->cancelByEvent(
                $subscriber->getData($emailKey),
        ...
        if ($this->isNeedToProcessSubscriptionRuleQueue($subscriber, $statusKey, $statusSubscribed, $statusUnsubscribed)) {//process rule queue if subscription status is active
            $ruleIds = Mage::getModel('followupemail/mysql4_rule')
    ...
    private function isNeedToProcessSubscriptionRuleQueue($subscriber, $statusKey, $statusSubscribed)
    {
        $result = false;
        if ($subscriber->getData($statusKey) === $statusSubscribed) {
            // Mage_Newsletter_Model_Subscriber
            if (method_exists($subscriber, 'getIsStatusChanged')) {
                $result = ($subscriber->getIsStatusChanged()) && ($subscriber->getData($statusKey) != $subscriber->getOrigData($statusKey));
            }
            // AW_Advancednewsletter_Model_Subscriber
            else {
                $result = true;
            }
        }
