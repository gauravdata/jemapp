...
class AW_Raf_Block_Sales_Order_Invoice_Totals extends Mage_Core_Block_Template
{
    public function initTotals()
    {
        ...
        if ($parent->getSource() instanceOf Mage_Sales_Model_Order) {
	    $orderSource = $parent->getSource();
	} else {
	    $orderSource = $parent->getSource()->getOrder();
	}
        $customer = Mage::getModel('customer/customer')->load($orderSource->getCustomerId());
        if (!$customer->getId()) {
            return;
        }
