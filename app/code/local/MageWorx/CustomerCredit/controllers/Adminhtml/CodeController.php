...
class MageWorx_Customercredit_Adminhtml_CodeController extends Mage_Adminhtml_Controller_Action
{
	...
	public function editAction() {
        try {
    	   $code = $this->_initCode();
            ...
            $block = $this->getLayout()->createBlock('customercredit/adminhtml_code_edit');
            
            $this->_title($this->__('Recharge Codes'))->_title($this->__('Manage'));
            
            $this->_initAction();
            $this->_addContent($block)
                ->_addLeft($this->getLayout()->createBlock('customercredit/adminhtml_code_edit_tabs'))
                ->renderLayout();
        }
        catch (Exception $e)
        {
    ...
    private function _generateContentCsv($existData = FALSE) {
        ...
        if($existData) {
            ...
            $modelCollection = Mage::getModel('customercredit/code')->getCollection();
            $dataList = array();
