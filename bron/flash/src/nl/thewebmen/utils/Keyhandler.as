package nl.thewebmen.utils
{
	...
	public class Keyhandler 
	{
		...
		/**
		 * Add the events to the stage
		 * @param	stage	The stage
		 */
		public static function startEvents(stage:Stage):void
		{
			stage.addEventListener(KeyboardEvent.KEY_UP, handleUp);
			stage.addEventListener(KeyboardEvent.KEY_DOWN, handleDown);
		}
		
		/**
		 * Remove the events from the stage
		 * @param	stage	The stage
		 */
		public static function stopEvents(stage:Stage):void
		{
		...
		{
			return keys[keycode] == undefined ? false : keys[keycode];
		}
		
		/**
		 * Get all keys that are down
		 * @return		Array with the keycodes that are down
		 */
		public static function getAllDown():Array
		{
			var a:Array = [];
		...
		public static function addKeyCallback(callback:Function, keycode:int):void
		{
			if (callbacks[keycode] == undefined) {
				callbacks[keycode] = [];
			}
			if (callbacks[keycode].indexOf(callback) == -1) {
				callbacks[keycode].push(callback);
			}
		}
		
		public static function removeKeyCallback(callback:Function, keycode:int):void
		{
			if (callbacks[keycode] != undefined && callbacks[keycode].indexOf(callback) != -1) {
				callbacks[keycode].splice(callbacks[keycode].indexOf(callback), 1);
			}
		}
		
		private static function allUp():void
		{
			keys = [];
		}
		
		private static function handleDown(e:KeyboardEvent):void 
		{
			keys[e.keyCode] = true;
			if (callbacks[e.keyCode] != undefined) {
				for each(var c:Function in callbacks[e.keyCode]) {
					c.call();
				}
			}
		}
		
		private static function handleUp(e:KeyboardEvent):void 
		{
